...
proxy.get('/api/reviews/all/:restaurantId', (req, res) => {
  ...
  axios.get(`http://3.83.40.199:9001/api/reviews/all/${restaurantId}`)
    .then(({data}) => {
      res.send(data);
    });
});
...
proxy.get('/api/reviews/summary/:restaurantId', (req, res) => {
  const { restaurantId } = req.params;
  axios.get(`http://3.83.40.199:9001/api/reviews/summary/${restaurantId}`)
    .then(({data}) => {
      res.send(data);
    });
});
...
proxy.get('/api/nav/:metro', (req, res) => {
  ...
  axios.get(`http://3.83.40.199:9004/api/nav/${metro}`)
    .then(({data}) => {
      res.send(data);
    });
});
...
proxy.get('/api/search/:searched', (req, res) => {
  ...
  axios.get(`http://3.83.40.199:9004/api/search/${searched}`)
    .then(({data}) => {
      res.send(data);
    });
});
...
proxy.get('/api/menus/:rest_id', (req, res) => {
  ...
  axios.get(`http://3.83.40.199:9003/api/menus/${rest_id}`)
    .then(({data}) => {
      res.send(data);
    });
});
...
proxy.get('/api/restaurant', (req, res) => {
  axios.get(`http://3.83.40.199:9002/api/restaurant`, {params: {id: 2}})
    .then(({data}) => {
      res.send(data);
    });
});
